...
def cortar(env, cliente):
    ...
    dt += tiempo_corte  # Acumula los tiempos de uso de la instalación
...
def cliente(env, name, personal):
    ...
    with personal.request() as request:  # Esperar turno
        ...
        print(f"{name}%s Pasa con peluquero en el minuto {round(pasa,2)} name, pasa, espera{round(pasa,2)}")
        yield env.process(cortar(env, name))  # Llamar al proceso
...
random.seed(semilla)
env = sympy.Environment()  # Creo el entorno de simulación
personal = sympy.Resource(env, num_peluqueros)  # Crea los recursos peluqueros
